...
def stats(values, statlist): # {{{
	...
	for x in values:
		...
		minx = min(minx, x)
		maxx = max(maxx, x)
		if S_MEDIAN in statlist or S_Q1 in statlist or S_Q3 in statlist:
			xvalues.append(x)
...
def main(): # {{{
	...
	try: ans = stats(line_reader(opts.infd), opts)
	except ValueError:
	...
	opts.outfd.write(' '.join(['%.5f' % ans[s] for s in opts.stats]))
	if opts.newline:
		opts.outfd.write('\n')
	return 0
# }}}
