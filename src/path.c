...
struct path * path_create_copy(const struct path *path) /* {{{ */
{
	...
	if(!newp) logea(__FILE__, __LINE__, NULL);
	newp->src = path->src;
	newp->dst = path->dst;
	...
	if(!newp->hops) logea(__FILE__, __LINE__, NULL);
	newp->ifaces = pavl_create(iface_cmp_ip_data, NULL, NULL);
	if(!newp->ifaces) logea(__FILE__, __LINE__, "pavl_create failed");

	for(int i = 0; i < newp->length; i++) {
		newp->hops[i] = pathhop_create_copy(path->hops[i]);
		if(!newp->hops[i]) logea(__FILE__, __LINE__, "unreachable");
...
struct path * path_create_str(const char *buf) /* {{{ */
{
	...
	if(r < 4) goto out;
	if(!inet_pton(AF_INET, src, &srcip)) goto out;
	if(!inet_pton(AF_INET, dst, &dstip)) goto out;
...
struct path * path_create_str_hops(const char *buf, uint32_t dst) /* {{{ */
{
	...
	if(!p) logea(__FILE__, __LINE__, NULL);
	...
	if(!p->ifaces) logea(__FILE__, __LINE__, NULL);
...

	{
		...
		for(curr = strsep(&ptr, "|"); curr; curr = strsep(&ptr, "|")) {
			hops[p->length] = pathhop_create_str(curr, p->tstamp,
					p->length);
			if(!hops[p->length]) continue;
			path_add_ifaces(p, hops[p->length]);
			p->length++;
		}
		if(p->length == 0) goto out_ifaces;
		p->hops = malloc(p->length * sizeof(struct pathhop *));
		if(!p->hops) logea(__FILE__, __LINE__, NULL);
...
struct path * path_create_str_safe(const char *buf, uint32_t dst) /* {{{ */
{
	...
	if(!inet_ntop(AF_INET, &dst, addr, INET_ADDRSTRLEN)) goto out;
...
void path_destroy(struct path *p) /* {{{ */
{
	...
	for(i = 0; i < p->length; i++) pathhop_destroy(p->hops[i]);
	free(p->hops);
	free(p);
} /* }}} */
...
char * path_tostr(const struct path *p) /* {{{ */
{
	...
	if(!inet_ntop(AF_INET, &p->src, src, INET_ADDRSTRLEN)) goto out;
	if(!inet_ntop(AF_INET, &p->dst, dst, INET_ADDRSTRLEN)) goto out;
		...
		uint8_t flowid, uint32_t ip)
{
	...
	if(ip == UINT32_MAX) return 0; /* no information in probe */
	if(hop == NULL) return 1; /* path has grown */
	if(pathhop_is_star(hop)) return 0; /* anything goes */
		...
		uint32_t flags)
{
	...
	for(int i = 0; i < p->length; i++) {
		if(pathhop_eq(p->hops[i], hop, flags)) return i;
	}
		...
		int oi, int ni, int *oj, int *nj, uint32_t flags)
{
	...
	for(nni = ni; nni < p2->length; nni++) {
		if(pathhop_is_star(p2->hops[nni])) continue;
		...
		struct path *p2, int i1, int i2, int j1, int j2)
{
	...
	h1 = p1->hops[i1];
	h2 = p2->hops[i2];
	if(pathhop_is_star(h1) && pathhop_is_star(h2)) return 1;
	if(!pathhop_is_star(h1) && !pathhop_is_star(h2)) return 0;
	...
	if(srch->nifaces > 1) return 0;
	/* not fixing with interface already in another hop: */
	if(pavl_find(starp->ifaces, srch->ifaces[0])) return 0;
	/* not fixing with dst if it's not the last hop in the path: */
	if(srch->ifaces[0]->ip == starp->dst && stari+1 != starj) return 0;
		...
		struct path *p2, int ttl)
{
	...
	if(!hops) logea(__FILE__, __LINE__, NULL);
...
static void path_set_hop(struct path *p, int ttl, struct pathhop *h) /* {{{ */
{
	...
	if(p->hops[ttl]) pathhop_destroy(p->hops[ttl]);
	p->hops[ttl] = h;
} /* }}} */
		...
		struct timespec tstamp, int ttl)
{
	...
	if(!hop) logea(__FILE__, __LINE__, NULL);
	...
	for(ifs = strsep(&ptr, ";"); ifs; ifs = strsep(&ptr, ";")){
		...
		if(!ifaces[hop->nifaces]) goto out_ifaces;
		hop->nifaces++;
	}
	hop->ifaces = malloc(hop->nifaces * sizeof(struct iface *));
	if(!hop->ifaces) logea(__FILE__, __LINE__, NULL);
	...
	for(i = 0; i < hop->nifaces; i++) iface_destroy(ifaces[i]);
	free(hop);
	free(buf);
	return NULL;
} /* }}} */
...
{
	int i;
	for(i = 0; i < h->nifaces; i++) iface_destroy(h->ifaces[i]);
	free(h->ifaces);
	free(h);
} /* }}} */
...
{
	if(h->nifaces > 1) return 0;
	if(h->ifaces[0]->ip != UINT32_MAX) return 0;
	return 1;
} /* }}} */
		...
		const struct pathhop *h2, uint32_t flags)
{
	if(flags & PATH_DIFF_FLAG_IGNORE_BALANCERS) {
		...
		int i;
		if(h1->nifaces != h2->nifaces) return 0;
		for(i = 0; i < h1->nifaces; i++) {
			if(h1->ifaces[i]->ip != h2->ifaces[i]->ip) return 0;
		}
	}
	return 1;
} /* }}} */
		...
		struct timespec tstamp, int ttl)
{
	...
	if(r < 6) goto out_iface;
	
	if(!inet_pton(AF_INET, addr, &iface->ip)) goto out_iface;
	...
	if(strlen(flags) > 0) {
		...
		if(!iface->flags) logea(__FILE__, __LINE__, NULL);
	}
...
{
	if(iface->flags) free(iface->flags);
	free(iface->flowids);
	free(iface);
} /* }}} */
...
char * iface_tostr(const struct iface *iface) /* {{{ */
{
	...
	if(!inet_ntop(AF_INET, &iface->ip, addr, INET_ADDRSTRLEN)) goto out;
...
struct pathdb * pathdb_create(int max_aliases) /* {{{ */
{
	...
	if(!db) logea(__FILE__, __LINE__, NULL);
	db->max_aliases = max_aliases;
	db->dst2entry = map_create(map_cmp_uint32, NULL, NULL);
	if(!db->dst2entry) logea(__FILE__, __LINE__, "map_create failed");
	return db;
} /* }}} */
...
static struct pathentry * pathdb_entry_create(uint32_t dst) /* {{{ */
{
	...
	if(!e) logea(__FILE__, __LINE__, NULL);
	e->dst = dst;
	e->maxalias = 0;
	e->dl = dlist_create();
	if(!e->dl) logea(__FILE__, __LINE__, "dlist_create failed");
	return e;
} /* }}} */
