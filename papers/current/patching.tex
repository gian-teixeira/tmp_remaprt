...
%we have 18 million of overlaps that match the query (i.e., has join in the
%overlap).
% In 23% of them, join is capable to detect a change in the
% overlapping path. If we consider overlaps in join ignoring the lczd size, we
% have 38M of overlaps detecting only 9% of changes. 
...
\paragraphnd{Finding paths that change.}  We find that 67\%
