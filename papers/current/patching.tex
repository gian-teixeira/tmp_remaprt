...
monitored paths. \ed{TODO FIX FIX}
...
overlapping path $q$, i.e., $\lczd(r')\cap{}q$. Although $O$ appears
to be a good set to probe in order to detect changes in an
overlapping path, it clearly misses changes in hops that do not
overlap with $\lczd(r')$ but are between hops in $O$, e.g.,
differences caused by load balancing.  Thus, we start with
a definition containing all radii between the first (closest to the
source) and last (furthest from the source) hops in $O(\lczd(r'),
q)$, denoted by \emph{candidate probing set} $\cps(q)$, which
detects all changes (if any) related to $O$ in an overlapping path.
